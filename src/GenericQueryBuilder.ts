export abstract class GenericQueryBuilder {
  ...
  private _prepare_for_limit_and_offset(item, type = 'limit') {
    ...
    if (typeof item === 'string') {
      ...
      if (!/^\d+$/.test(item)) {
        throw new Error(
          `The string you provided to ${type} by contains non-integer values--this isn't allowed.`
        );
      }
      // Force to an integer
      item = parseInt(item);
    ...
    if (typeof item === 'number') {
      ...
      if (item % 1 !== 0) {
        throw new Error(
          `You cannot ${type} a SQL resultset with a floating point value!`
        );
      }
    } else {
      throw new Error(
        `There was an unrecoverable error while parsing the value provided in your ${type} statement.`
      );
    }
  ...
  private _escape_identifiers(item: {} | string = '*', split = true) {
    ...
    if (
      ...
      for (let i in item as {}) {
        item[i] = this._escape_identifiers_string(item[i]);
      }
      return item;
    } else {
      return this._escape_identifiers_string(item as string, split);
    }
  }
  ...
  _protect_identifiers(
    ...
    if (
      ...
      for (let k in item) {
        const v = item[k];
        escaped_array[this._protect_identifiers(k)] =
          this._protect_identifiers(v);
      }
    ...
    if (/\sAS\s/gi.test(item)) {
      ...
      alias =
        protect_identifiers &&
        !this._has_operator(item.substring(alias_index + 1))
          ? ' ' + this._escape_identifiers(item.substring(alias_index + 1))
          : item.substring(alias_index);
      item = item.substring(0, alias_index);
    }
  ...
  private _has_operator(str) {
    if (typeof str === 'string' && str.length > 0) {
      const match =
        /(<|>|!|=|\sIS NULL|\sIS NOT NULL|\sEXISTS|\sBETWEEN|\sLIKE|\sCASE|\sTHEN|\sWHEN|\sIN\s*\(|\s)/i.test(
          str.trim()
        );
      if (!match) {
        return false;
  ...
  private _where(key, value = null, type = 'AND ', escape = true) {
    ...
    if (Object.keys(key).length === 0) {
      throw new Error(
        "where(): You haven't provided any key value pairs to limit the resultset by."
      );
    }
  ...
  /**
   *
   * @param field string if field name or Object with pair name as key match value as Value
   * @param match string if field name or Object with pair name as key match value as Value
   * @param side Side to mathc it with
   * @returns
   */
  like(
    field: string | GenericJSONInput,
    match: MatchInputValues | GenericJSONInput,
    side?: LikeSides
  ) {
  ...
  private _like(
    ...
    for (let k in field) {
      ...
      } else if (['after', 'right', 'left', 'before', 'both'].includes(side)) {
        if (typeof v === 'string') {
          like_statement =
            k +
            not +
  ...
  private _min_max_avg_sum(select = '', alias = '', type = 'MAX') {
    ...
    const sql = `${type}(${this._protect_identifiers(
      select.trim()
    )}) AS ${alias}`;
  ...
  private _having(key, value, type = 'AND ', escape = true) {
    ...
    if (/^(string|number|boolean)$/.test(typeof value)) {
      ...
      throw new Error(
        `Improper value supplied as the second parameter (${typeof value}) of the having() method.`
      );
    }
  ...
  join(table = '', relation = '', direction = '', escape = true) {
    ...
    if (escape === true && this.multi_condition_rgx.test(relation)) {
      const new_relation = relation
        .split(this.multi_condition_rgx)
        .map((v, i) => {
          if (i % 2 !== 0) return v.trim();
          const match = v.match(this.condition_rgx);
          if (!match) return v.trim();
          return `${this._protect_identifiers(
            match[1].trim(),
            false,
            escape
          )} ${match[2].trim()} ${this._protect_identifiers(
            match[3].trim(),
            false,
            escape
          )}`;
    ...
    // Split apart the condition and protect the identifiers
      ...
      relation &&
      escape !== false &&
      this.condition_rgx.test(relation)
    ) {
      const match = relation.match(this.condition_rgx);
      relation = `ON ${this._protect_identifiers(
        match[1].trim(),
        false,
        escape
      )} ${match[2].trim()} ${this._protect_identifiers(
      ...
      relation = `USING (${
        escape ? this._escape_identifiers(relation) : relation
      })`;
  ...
  order_by(orderby, direction?) {
    ...
    for (let i in orderby) {
      ...
      if ((m = orderby[i].match(/([^\s]+)\s+(ASC|DESC|RAND\(\)|NEWID\(\))/i))) {
        ...
        orderby[i] = {
          field: this._protect_identifiers(m[1].trim()),
        ...
        if (/^(ASC|DESC)$/i.test(direction) || direction === '') {
          ...
          throw new Error(
            "Invalid direction provided in order_by method! Only 'ASC', 'DESC', and 'RAND' are allowed!"
          );
        }
      }
  ...
  private set(key, value?, escape = true) {
    ...
    for (let i in key) {
      ...
      if (
        !/^(number|string|boolean)$/.test(typeof v) &&
        v !== null &&
        escape === false
      ) {
        throw new Error(
  ...
  _update(table = '', set = null, where = null) {
    ...
    if (Array.isArray(set)) {
      ...
      if (set.length > 0) {
        if (
          Object.prototype.toString.call(set[0]) ===
          Object.prototype.toString.call({})
        ) {
  ...
  private compile_update(table) {
    return this.get_compiled_update(table);
  }

  private compile_insert(table) {
  ...
  _update_batch(table = '', set = null, index = null, where = null) {
    ...
    if (this.from_array.length !== 1) {
      if (this.from_array.length === 0) {
        throw new Error(
          "You haven't provided any tables to build batch UPDATE query with!"
        );
      }
      throw new Error(
    ...
    for (let i = 0, total = this.set_array.length; i < total; i += 100) {
      ...
      for (let j in chunk) {
        ...
        for (let k in keys) {
          if (keys[k] != index) {
            ...
            when_then[keys[k]].push(
              `WHEN ${index} = ${ids[j]} THEN ${chunk[j][keys[k]]} `
            );
          }
        }
      }
      ...
      sql += cases.substring(0, cases.length - 2);
  ...
  abstract _compile_delete();
}

export type MatchInputValues = string | boolean | number | bigint;
export interface GenericJSONInput {
  [key: string]: MatchInputValues;
}
export type LikeSides = 'both' | 'left' | 'before' | 'after' | 'right' | 'none';
